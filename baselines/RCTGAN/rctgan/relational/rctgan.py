...
class RCTGAN:
    def __init__(self, metadata=None, hyperparam=None, current_table=None,
                 ohe_for_parent=False, if_gaussian_ht=True, num_transformers='gaussian',
                 seed=None):
    ...
    def rdt2_transform(self, meta_fields, table,
                       field_deleted=[]):  # meta_fields = metadata.get_table_meta('store')['fields']
        ht = HyperTransformer()
            ...
            if field not in field_deleted:
                if meta_fields[field]['type'] == 'categorical':
                    ...
                    if self.num_transformers == 'gaussian':
                        ...
                        config_dict["transformers"][field] = FloatFormatter(
                    ...
                    config_dict["transformers"][field] = OptimizedTimestampEncoder(
                        missing_value_replacement='mean', datetime_format=format_datetime)
    ...
    def transform(self, table_name, table):
        ...
        if self.if_gaussian_ht == False:
            ...
            return self.transformers[table_name]['gaussian_ht'].transform(
                self.transformers[table_name]['hypertr'].transform(table[col]))
    ...
    def parents_input_add(self, table_name, tables, parents_trandformed=None):
        ...
        for parent_name in parents_name:
            ...
            for foreign_key in foreign_keys:
                ...
                parent_trandformed.columns = ["var_" + str(i) for i in
                                              range(1, len(parent_trandformed.columns) + 1)]
                if self.hyperparam[self.current_table]["grand_parent"]:
                    if parent_name in list(self.metadata.get_parents(self.current_table)):
                        parent_trandformed = self.parents_input_add(parent_name, tables,
                                                                    parent_trandformed)
                parent_trandformed.columns = ["var_" + str(count + i) for i in
                                              range(1, len(parent_trandformed.columns) + 1)]
        ...
        for table_name in self.metadata.get_tables():
            ...
            if len(children) > 0:
                ...
                if self.if_gaussian_ht:
                    ...
                    col = self.transformers[table_name]['columns']
                    self.transformers[table_name]['gaussian_ht'] = self.gaussian_ht(
                        ht.transform(tables[table_name][col]))
        ...
        for table_name in self.metadata.get_tables():
            ...
            if len(self.metadata.get_parents(table_name)) == 0:
                ...
                model = CTGAN(primary_key=prim_key,
                              ...
                              discriminator_decay=self.hyperparam[table_name][
                                  "discriminator_decay"],
                              batch_size=self.hyperparam[table_name]["batch_size"],
                              discriminator_steps=self.hyperparam[table_name][
                                  "discriminator_steps"],
                ...
                for child_name in children:
                    ...
                    for foreign_key in foreign_keys:
                        temp_child = pd.DataFrame(
                            tables[child_name].groupby([foreign_key])[foreign_key].count())
                        ...
                        temp_table[child_name + "_" + foreign_key + "_nb_occ"] = temp_table[
                            child_name + "_" + foreign_key + "_nb_occ"].astype(int)
                        self.size_stats[table_name][child_name + "_" + foreign_key + "_nb_occ"] = {
                            "min": np.min(temp_table[child_name + "_" + foreign_key + "_nb_occ"]),
                            "max": np.max(temp_table[child_name + "_" + foreign_key + "_nb_occ"]),
                            "mean": np.mean(
                                temp_table[child_name + "_" + foreign_key + "_nb_occ"]),
                ...
                model = PC_CTGAN(embedding_dim=self.hyperparam[table_name]["embedding_dim"],
                                 generator_dim=self.hyperparam[table_name]["generator_dim"],
                                 discriminator_dim=self.hyperparam[table_name][
                                     "discriminator_dim"],
                                 generator_lr=self.hyperparam[table_name]["generator_lr"],
                                 generator_decay=self.hyperparam[table_name]["generator_decay"],
                                 discriminator_lr=self.hyperparam[table_name]["discriminator_lr"],
                                 ...
                                 plot_loss=self.hyperparam[table_name]["plot_loss"],
                                 seed=self.seed,
                                 field_transformers=self.hyperparam[table_name][
                                     "field_transformers"],
                                 anonymize_fields=self.hyperparam[table_name]["anonymize_fields"],
                                 constraints=self.hyperparam[table_name]["constraints"],
                                 table_metadata=self.hyperparam[table_name]["table_metadata"],
                    ...
                    for child_name in children:
                        ...
                        for foreign_key in foreign_keys:
                            ...
                            temp_table[child_name + "_" + foreign_key + "_nb_occ"] = temp_table[
                                child_name + "_" + foreign_key + "_nb_occ"].astype(int)
                            self.size_stats[table_name][
                                child_name + "_" + foreign_key + "_nb_occ"] = {"min": np.min(
                                temp_table[child_name + "_" + foreign_key + "_nb_occ"]),
                                                                               "max": np.max(
                                                                                   temp_table[
                                                                                       child_name + "_" + foreign_key + "_nb_occ"]),
                                                                               "mean": np.mean(
                                                                                   temp_table[
                                                                                       child_name + "_" + foreign_key + "_nb_occ"]),
                                                                               "std": np.std(
    ...
    def parent_child_sample_mini(self, child, sampled_data, table_transformed, f_key_frame):
        sampled_data[child] = self.models[child].sample(list(f_key_frame["_size_"]),
                                                        table_transformed)
        ...
        sampled_data[child] = sampled_data[child].merge(f_key_frame,
                                                        ...
                                                        how='left',
                                                        indicator=True)
        sampled_data[child] = sampled_data[child].drop(["_size_", "Parent_index", "_merge"],
                                                       axis=1)
    ...
    def granp_parent_transform_add(self, parent_name, table_transformed, f_key, f_key_frame,
                                   sampled_data, tables_transformed):
        ...
        for grand_parent in grand_parents:
            ...
            for gp_foreign_key in gp_foreign_keys:
                ...
                temp_table.columns = ["var_" + str(start_var + i + 1) for i in
                                      range(len(temp_table.columns))]
                ...
                temp_serie = temp_serie.merge(temp_table, on=[gp_foreign_key], how='left',
                                              indicator=True)
                ...
                table_transformed = table_transformed.merge(temp_serie, on=[f_key], how='left',
                                                            indicator=True)
    ...
    def parent_child_sample(self, child, sampled_data, tables_transformed):
        ...
        if all_parents_sampled:
            ...
            table_transformed.columns = ["var_" + str(i + 1) for i in
                                         range(len(table_transformed.columns))]
            ...
            if self.hyperparam[child]["grand_parent"]:
                table_transformed = self.granp_parent_transform_add(enc_parent, table_transformed,
                                                                    enc_foreign_key, f_key_frame,
                                                                    sampled_data,
                                                                    tables_transformed)

            if len(parents_name) == 1 and len(
                list(self.metadata.get_foreign_keys(enc_parent, child))) == 1:
                ...
                for parent_name in parents_name:
                    ...
                    for foreign_key in foreign_keys:
                        start_var = len(table_transformed.columns)
                        indexes_chosen = list(random.choices(tables_transformed[parent_name].index,
                                                             k=len(table_transformed)))
                        random_parent_rows_transformed = tables_transformed[parent_name].loc[
                            indexes_chosen].copy()
                        random_parent_rows_transformed.columns = ["var_" + str(start_var + i + 1)
                                                                  for i in range(
                                len(random_parent_rows_transformed.columns))]
                        random_parent_rows_transformed.index = range(
                            len(random_parent_rows_transformed))
                        table_transformed = pd.concat(
                            [table_transformed, random_parent_rows_transformed], axis=1)
                        del random_parent_rows_transformed

                        prim_key = self.metadata.get_primary_key(parent_name)
                        f_key_frame[foreign_key] = list(
                            sampled_data[parent_name].loc[indexes_chosen][prim_key])
                        if self.hyperparam[child]["grand_parent"]:
                            table_transformed = self.granp_parent_transform_add(parent_name,
                                                                                table_transformed,
                                                                                foreign_key,
                                                                                f_key_frame,
                                                                                sampled_data,
