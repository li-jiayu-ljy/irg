...
def quantile_normalize_sklearn(matrix):
    transformer = QuantileTransformer(output_distribution='normal',
                                      random_state=42)  # Change output_distribution as needed
...
def handle_multi_parent(
    ...
    for df, parent in synthetic_child_dfs:
        ...
        cat_columns = []
        for c in df_without_ids.columns:
            if pd.to_numeric(df_without_ids[c].dropna(), errors="coerce").isna().any():
                cat_columns.append(c)
        # cat_columns = df_without_ids.select_dtypes(exclude=np.number).columns.tolist()
        combined = oe.fit_transform(
            pd.concat(
                [df_without_ids[cat_columns], anchor_df_without_ids[cat_columns]], ignore_index=True
            ).astype(str)
        )
        df_val = np.concatenate([
            df_without_ids.drop(columns=cat_columns).values.astype(float), combined[:df_without_ids.shape[0]]
        ], axis=1)
        anchor_val = np.concatenate([
            anchor_df_without_ids.drop(columns=cat_columns).values.astype(float), combined[df_without_ids.shape[0]:]
        ], axis=1)
